...
| 설정 | 값 | 근거 |
|------|---|------|
| maximum-pool-size | 5 | 부하테스트에서 모든 지표 최고 성능 |
| minimum-idle | 5 | pool size와 동일하게 유지하여 커넥션 생성/해제 오버헤드 제거 |
| connection-timeout | 3000ms | 커넥션 확보 실패 시 빠르게 에러 반환 |

### 선택 근거

- Redis가 대부분의 트래픽을 흡수하여 DB에 도달하는 요청이 실제 결제 건수(최대 10건)로 제한됨
- pool size를 키워도 성능이 향상되지 않고, 오히려 DB 측 유휴 커넥션 관리 비용으로 성능 저하
- 서버 2대 × 5 = 총 10 커넥션으로 MySQL `max_connections=151`의 7%만 사용하여 충분한 여유 확보

**트레이드오프**

- Redis 장애로 DB Fallback 전환 시, 모든 요청이 DB 비관적 락을 거치므로 pool size 5로는 커넥션 대기가 발생할 수 있음
- 단, 재고 10개가 빠르게 소진되어 락 경합 시간이 짧고, connection-timeout 3초로 대기 시간을 제한하므로 실질적 영향은 제한적
